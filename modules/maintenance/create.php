...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($description)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO maintenance_tasks (description, priority, proposed_solution, estimated_cost, status, assigned_to, is_recurring, frequency, next_due_date, start_time, end_time, show_on_portal, impact) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$description, $priority, $proposed_solution, $estimated_cost ?: 0, $status, $assigned_to, $is_recurring, $frequency, $next_due_date, $start_time, $end_time, $show_on_portal, $impact]);

            // Log
            $logStmt = $pdo->prepare("INSERT INTO audit_logs (user_id, action, details) VALUES (?, ?, ?)");
            $logStmt->execute([$_SESSION['user_id'], 'CREATE_TASK', "Created Task: $description"]);
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-3xl font-bold text-slate-800">Schedule Task</h1>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 border border-slate-200">
        ...
        <form method="POST" action="" x-data="{ recurring: false }">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Description -->
                <div class="col-span-2">
                    <label class="block text-slate-700 text-sm font-bold mb-2" for="description">Task Description
                        *</label>
                    <input type="text" name="description" id="description" placeholder="e.g. Monthly Server Reboot"
                        required
                ...
                <!-- Status -->
                <div>
                    <label class="block text-slate-700 text-sm font-bold mb-2" for="status">Initial Status</label>
                ...
                <div class="col-span-2 bg-slate-50 p-4 rounded border border-slate-200">
                    <div class="flex items-center mb-4">
                        <input type="checkbox" name="is_recurring" id="is_recurring" x-model="recurring"
                ...
                <div class="col-span-2 bg-indigo-50 p-4 rounded border border-indigo-100 mt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="show_on_portal" id="show_on_portal"
                        ...
                        <div>
                            ...
                            <select name="impact" id="impact"
                                ...
                                <option value="none">None (Routine)</option>
                                <option value="low">Low (Degraded Perf.)</option>
                                <option value="medium">Medium (Partial Outage)</option>
                                <option value="high">High (Significant Affect)</option>
                                <option value="outage">OUTAGE (All Staff Affected)</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="start_time" id="start_time"
                                class="w-full px-4 py-2 border border-indigo-200 rounded-lg focus:ring-indigo-500 outline-none text-sm">
                        </div>
                        <div>
