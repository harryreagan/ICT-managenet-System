...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($service_name) || empty($renewal_date)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE renewals SET service_name=?, vendor_id=?, contact_details=?, amount_paid=?, renewal_date=?, status=?, notes=?, billing_cycle=?, is_recurring=?, payment_status=? WHERE id=?");
            $stmt->execute([$service_name, $vendor_id, $contact_details, $amount_paid, $renewal_date, $status, $notes, $billing_cycle, $is_recurring, $payment_status, $id]);
            $_SESSION['success'] = "Renewal record updated successfully!";
            redirect('/ict/modules/renewals/index.php');
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-3xl font-bold text-slate-800">Edit Renewal</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="service_name" id="service_name"
                        value="<?php echo htmlspecialchars($renewal['service_name']); ?>"
                ...
                <div>
                    ...
                    <select name="vendor_id" id="vendor_id"
                        ...
                        <?php foreach ($vendors as $vendor): ?>
                            <option value="<?php echo $vendor['id']; ?>" <?php echo $renewal['vendor_id'] == $vendor['id'] ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="text" name="contact_details" id="contact_details"
                        value="<?php echo htmlspecialchars($renewal['contact_details']); ?>"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount_paid" id="amount_paid"
                        value="<?php echo htmlspecialchars($renewal['amount_paid']); ?>"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm"
                ...
                <div>
                    ...
                    <input type="date" name="renewal_date" id="renewal_date"
                        value="<?php echo htmlspecialchars($renewal['renewal_date']); ?>" required
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm bg-white">
                        <option value="active" <?php echo $renewal['status'] == 'active' ? 'selected' : ''; ?>>Active
                        </option>
                        <option value="expired" <?php echo $renewal['status'] == 'expired' ? 'selected' : ''; ?>>Expired
                        </option>
                        <option value="cancelled" <?php echo $renewal['status'] == 'cancelled' ? 'selected' : ''; ?>>
                            Cancelled</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="billing_cycle" id="billing_cycle"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm bg-white">
                        <option value="yearly" <?php echo ($renewal['billing_cycle'] ?? 'yearly') == 'yearly' ? 'selected' : ''; ?>>
                            Yearly</option>
                        <option value="monthly" <?php echo ($renewal['billing_cycle'] ?? 'yearly') == 'monthly' ? 'selected' : ''; ?>>
                            Monthly</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="payment_status" id="payment_status"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm bg-white">
                        <option value="unpaid" <?php echo ($renewal['payment_status'] ?? 'unpaid') == 'unpaid' ? 'selected' : ''; ?>>
                            Unpaid</option>
                        <option value="paid" <?php echo ($renewal['payment_status'] ?? 'unpaid') == 'paid' ? 'selected' : ''; ?>>Paid
                        </option>
                    </select>
                </div>
                ...
                <div class="flex items-center space-x-2 mt-2">
                    <input type="checkbox" name="is_recurring" id="is_recurring" value="1" <?php echo !empty($renewal['is_recurring']) ? 'checked' : ''; ?>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm"><?php echo htmlspecialchars($renewal['notes']); ?></textarea>
                </div>
            </div>
