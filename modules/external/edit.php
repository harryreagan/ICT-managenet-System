...
$stmt = $pdo->prepare("SELECT * FROM external_systems WHERE id = ?");
$stmt->execute([$id]);
$system = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($url)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE external_systems SET name=?, url=?, notes=?, owner=? WHERE id=?");
            $stmt->execute([$name, $url, $notes, $owner, $id]);
            $_SESSION['success'] = "External system updated successfully!";
            redirect('/ict/modules/external/index.php');
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-3xl font-bold text-slate-800">Edit System Link</h1>
        ...
        <form method="POST" action="">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="<?php echo htmlspecialchars($system['name']); ?>"
                        required
                ...
                <div class="col-span-2">
                    ...
                    <input type="url" name="url" id="url" value="<?php echo htmlspecialchars($system['url']); ?>"
                        required
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="owner" id="owner" value="<?php echo htmlspecialchars($system['owner']); ?>"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="2"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-primary-500 outline-none text-sm"><?php echo htmlspecialchars($system['notes']); ?></textarea>
                </div>
            </div>
