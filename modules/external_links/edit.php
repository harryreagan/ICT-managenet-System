...
$pageTitle = "Edit External System";
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($url)) {
        ...
        $stmt = $pdo->prepare("UPDATE external_links SET name = ?, url = ?, category = ? WHERE id = ?");
        $stmt->execute([$name, $url, $category, $id]);
        header("Location: index.php");
        exit();
    }
}
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="saas-card p-8">
        <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit System</h1>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="category"
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all">
                    <option value="Network" <?php echo $link['category'] == 'Network' ? 'selected' : ''; ?>>Network
                        Infrastructure</option>
                    <option value="Security" <?php echo $link['category'] == 'Security' ? 'selected' : ''; ?>>Security &
                        Surveillance</option>
                    <option value="Vendor" <?php echo $link['category'] == 'Vendor' ? 'selected' : ''; ?>>Vendor Portal
                    </option>
                    <option value="Other" <?php echo $link['category'] == 'Other' ? 'selected' : ''; ?>>Other Tools
                    </option>
                </select>
            </div>
