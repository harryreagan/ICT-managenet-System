...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $pdo->prepare("
        ...
        SET device_name = ?, device_type = ?, ip_address = ?, location = ?, 
            network_id = ?, mac_address = ?, manufacturer = ?, model = ?, notes = ?, status = ?
        WHERE id = ?
    ");
    ...
    $_SESSION['success'] = "Static device updated successfully!";
    redirect('/ict/modules/networks/static_devices.php');
}

// Fetch device data
$stmt = $pdo->prepare("SELECT * FROM static_devices WHERE id = ?");
$stmt->execute([$id]);
$device = $stmt->fetch();
...
<div class="max-w-3xl mx-auto fade-in-up">
    <div class="flex items-center space-x-3 mb-6">
        ...
        <h1 class="text-3xl font-bold text-slate-800">Edit Static Device</h1>
    </div>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="device_type" required
                    class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                    <option value="Printer" <?php echo $device['device_type'] === 'Printer' ? 'selected' : ''; ?>>Printer
                    </option>
                    <option value="POS" <?php echo $device['device_type'] === 'POS' ? 'selected' : ''; ?>>POS System
                    </option>
                    <option value="Scanner" <?php echo $device['device_type'] === 'Scanner' ? 'selected' : ''; ?>>Scanner
                    </option>
                    <option value="IP Phone" <?php echo $device['device_type'] === 'IP Phone' ? 'selected' : ''; ?>>IP
                        Phone</option>
                    <option value="Camera" <?php echo $device['device_type'] === 'Camera' ? 'selected' : ''; ?>>Camera
                    </option>
                    <option value="Access Point" <?php echo $device['device_type'] === 'Access Point' ? 'selected' : ''; ?>>Access Point</option>
                    <option value="Computer/PC" <?php echo $device['device_type'] === 'Computer/PC' ? 'selected' : ''; ?>>
                        Computer/PC</option>
                    <option value="Other" <?php echo $device['device_type'] === 'Other' ? 'selected' : ''; ?>>Other
                    </option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="network_id"
                        ...
                        <?php foreach ($networks as $net): ?>
                            <option value="<?php echo $net['id']; ?>" <?php echo $device['network_id'] == $net['id'] ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="text" name="mac_address"
                        value="<?php echo htmlspecialchars($device['mac_address'] ?? ''); ?>"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="manufacturer"
                        value="<?php echo htmlspecialchars($device['manufacturer'] ?? ''); ?>"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">
                    ...
                    <input type="text" name="model" value="<?php echo htmlspecialchars($device['model'] ?? ''); ?>"
                        class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="status" required
                    class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                    <option value="online" <?php echo $device['status'] === 'online' ? 'selected' : ''; ?>>Online
                    </option>
                    <option value="offline" <?php echo $device['status'] === 'offline' ? 'selected' : ''; ?>>Offline
                    </option>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                    class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none resize-none"><?php echo htmlspecialchars($device['notes'] ?? ''); ?></textarea>
            </div>
